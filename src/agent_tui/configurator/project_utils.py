...
class ProjectContext:
    ...
    def project_skills_dir(self) -> Path | None:
        """Return the project `.agent-tui/skills` directory, if any."""
        if self.project_root is None:
            return None
        return self.project_root / ".agent-tui" / "skills"

    def project_agents_dir(self) -> Path | None:
        """Return the project `.agent-tui/agents` directory, if any."""
        if self.project_root is None:
            return None
...
def find_project_agent_md(project_root: Path) -> list[Path]:
    ...
    1. project_root/.agent-tui/AGENTS.md
    ...
    """
    candidates = [
        project_root / ".agent-tui" / "AGENTS.md",
